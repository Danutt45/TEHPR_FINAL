...
router.route("/commentsfrombug/:id_bug").get((req, res) => {
  sequelize
    .query("SELECT id_proiect FROM teams WHERE id_user = :status", {
    ...
    .then((result) => {
      ...
      } else return res.json({ message: "user is not a project member" });
    })
...
router.route("/bugs").post((req, res) => {
  sequelize
    .query("SELECT id_proiect FROM testers WHERE id_user = :status", {
      replacements: { status: req.body.id_user },
      type: sequelize.SELECT,
    })
    .then((result1) => {
      ...
      } else return res.json({ message: "user is not a tester" });
    })
    .catch((err) => res.json(err.toString()));
});
...
router.route("/login").post((req, res) => {
  User.findAll({
    where: { mail: req.body.mail, parola: req.body.parola },
  }).then((usr) => {
    if (usr != 0) {
